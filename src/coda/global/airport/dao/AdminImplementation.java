...
public class AdminImplementation implements AdminInterface {
	...
	public String addFlight(Flight flight) throws SQLException {
		...
		PreparedStatement addFlight = con
				.prepareStatement("insert into flight (flight_no,airline,boarding_place,destination) values (?,?,?,?)");
	...
	public String addSeats(Flight flight) throws SQLException {
		// viewAddedFlight();
		Database db = new Database();
		Connection con = db.getConnection();
		String result = "";
		...
		try {
			...
			if (!leaveApprove.execute()) {
				...
				PreparedStatement updateAvailablity = con
						.prepareStatement("insert into crew_availability values (?,?,?)");
	...
	public List<Crew> slotAssign(int scheduledFlight) {
		...
		try {
			...
			while (compensationList.next()) {
				...
				// PreparedStatement updateAvailablity = con
				// .prepareStatement("update crew_availability set status=? where crew_id =? and
				// date=?");
				// updateAvailablity.setString(1, "NA");
				// updateAvailablity.setInt(2, crew_id);
				// updateAvailablity.setDate(3, new java.sql.Date(date.getTime()));
			...
			// } else if (scan.next().equalsIgnoreCase("N")) {
			// PreparedStatement updateAvailablity = con
			// .prepareStatement("update slot_requests set status=? where crew_id =? and
			// flight_schedule_no=?");
			...
			PreparedStatement moreCrew = con.prepareStatement(
					"select * from crew c where  c.id!=ALL(select slot.crew_id from slot where c.id=slot.crew_id and  flight_schedule_no=?);");
			moreCrew.setInt(1, scheduledFlight);
			ResultSet moreCrewSet = moreCrew.executeQuery();
			while (moreCrewSet.next()) {
				Crew crew = new Crew();
				PreparedStatement checkAvail = con
						.prepareStatement("select * from crew_availability ca where ca.crew_id=? and ca.status!=?");
	...
	public void denySlot(int schedule, int crewId, String type) {
		...
		case "NON":
			...
			try {
				updateAvailablity = con.prepareStatement(
						"update slot_requests set status=? where crew_id =? and flight_schedule_no=?");
				updateAvailablity.setString(1, "REJ");
				updateAvailablity.setInt(2, crewId);
